  ...
  async execute(interaction) {
    ...
    if (invalidRoles.length > 0) {
      return interaction.reply({ 
        content: `❌ You cannot assign roles equal or higher than your highest role: ${invalidRoles.map(r => r.name).join(', ')}`, 
        ephemeral: true 
      });
    }
    ...
    if (successfulRoles.length > 0) {
      description += `✅ Added roles: ${successfulRoles.map(r => `**${r.name}**`).join(', ')} to ${user}.\n`;
    }
